...
alter table public.recordings
  drop constraint if exists recordings_source_type_check;

alter table public.recordings
  add constraint recordings_source_type_check
  check (source_type in ('microphone', 'upload', 'desktop_meeting'));

create index if not exists recordings_capture_source_app_idx
  on public.recordings ((capture_metadata ->> 'sourceApp'));


create index if not exists recordings_capture_platform_idx
  on public.recordings ((capture_metadata ->> 'platform'));
...
as $$
  select jsonb_build_object(
    ...
    'captureMetadata', r.capture_metadata,
